...
import {
	PackageExportPathContext,
	allExportPathCombinations,
} from '../export/auto-export.class.js';
...
export class AutoBin implements AutolibPlugin {
	public static readonly featureName = 'bin';
	...
	constructor(context: NormalizedAutolibContext, options?: AutoBinOptions) {
	...
	private async createShims(shimPaths: string[], format: InternalModuleFormat): Promise<void> {
		if (
			...
			const shimPathsToMake = await Promise.all(
				shimPaths.map((path) =>
					readFile(toAbsolute(path, this.context), { encoding: 'utf8' })
						.then((content) =>
