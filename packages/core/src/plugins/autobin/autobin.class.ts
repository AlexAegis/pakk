...
import { NormalizedAutolibContext, ViteFileNameFn } from '../../index.js';
import {
	NPM_INSTALL_HOOKS,
...
import { PackageExportPathContext, allExportPathCombinations } from '../entry/auto-export.class.js';
import { createExportMapFromPaths } from '../entry/helpers/create-export-map-from-paths.function.js';
...
export class AutoBin implements AutolibPlugin {
	...
	constructor(options: AutoBinOptions, context: NormalizedAutolibContext) {
	...
	async examinePackage(
		...
		for (const binPath of binFiles) {
			...
			this.binPathMap[binName] = {
				...
				'development-to-dist': join(this.options.outDir, binPath),
